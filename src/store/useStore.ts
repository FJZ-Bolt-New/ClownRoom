...
// NEW: Memory image storage interface
...
interface AppState {
  ...
  deleteMemory: (memoryId: string) => void; // NEW: Delete memory function
  ...
  // FIXED: Memory like/dislike functionality that works for ALL memories (including mock ones)
...
// NEW: Separate storage for memory images using IndexedDB-like approach
...
const saveMemoryImagesToStorage = (memoryImages: MemoryImageStorage) => {
  try {
    // Use a separate localStorage key for images to avoid quota issues with main store
    ...
    try {
      ...
      // Remove oldest image entries if we have too many
  ...
  persist(
    (set, get) => ({
      ...
      addMemory: (memory) => set((state) => {
        // NEW: Store images separately if they contain base64 data
        if (memory.mediaUrls && memory.mediaUrls.some(url => url.startsWith('data:'))) {
          ...
          // Store memory with placeholder URLs
          ...
          // Regular memory without base64 images
          return {
      ...
      // NEW: Memory image storage functions
    ...
    }),
    {
      ...
      version: 5, // FIXED: Increment version to handle migration
      partialize: (state) => ({
        ...
        // FIXED: Store memories without large base64 data
        memories: state.memories,
        userVotes: state.userVotes,
        userLikes: state.userLikes,
        userDislikes: state.userDislikes,
        userReactions: state.userReactions,
        userMemoryLikes: state.userMemoryLikes, // FIXED: Persist memory likes
        userMemoryDislikes: state.userMemoryDislikes, // FIXED: Persist memory dislikes
        ...
        // NOTE: memoryImages are stored separately to avoid localStorage quota issues
      }),
      serialize: (state) => {
        try {
      ...
      deserialize: (str) => {
        try {
          ...
          // FIXED: Ensure memory like/dislike tracking exists
      ...
      onRehydrateStorage: () => (state) => {
        if (state) {
          ...
          // Load memory images from separate storage
